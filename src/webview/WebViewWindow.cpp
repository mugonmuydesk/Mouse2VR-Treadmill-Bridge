...
#ifdef USE_EXTERNAL_HTML
    // Use the HTML from the generated header file
    // This allows editing HTML/CSS/JS in separate files
    return Mouse2VR::WebView::GetEmbeddedHTML();
#else
    // Original embedded HTML (kept for backward compatibility)
    // To use external HTML files:
    // 1. Run scripts/extract_html.py to extract current HTML
    // 2. Edit files in src/webview/ui/
    // 3. Run scripts/inline_html.py to generate header
    // 4. Uncomment #define USE_EXTERNAL_HTML above
